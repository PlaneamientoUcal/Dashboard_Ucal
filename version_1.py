...
def load_data():
    ...
    df = pd.read_excel("2025-06-19_bbdd_ucal_['2025-2']_conv_(0,1)_pagantes_(0,1)_fecha_250619.xlsx", engine='openpyxl')
    df_261 = pd.read_excel("2025-06-19_bbdd_ucal_['2026-1']_conv_(0,1)_pagantes_(0,1)_fecha_250619.xlsx", engine='openpyxl')
    try:
        data2 = pd.read_csv("250619bbdd_ucal2026-1','2025-2.csv", sep=',', dtype=str)
        data2.columns = data2.columns.str.strip().str.replace(' ', '_')
    except Exception as e:
        st.error(f"Error cargando data2: {e}")
    data_espejo = pd.read_csv("250619bbdd_ucal2025-1', '2024-2.csv", sep=',', dtype=str)
